...
def index(request):
    ...
    if 'cart' in request.session:
        for c in request.session['cart']:
            ...
            price = m.daily_plan_price if c.get('plan') == 1 else m.weekly_plan_price if c.get('plan') == 2 else m.monthly_plan_price if c.get('plan') == 3 else None
...
def restaurant_detail(request, pk):
    ...
    restaurant_order_ids = orders.objects.filter(
        order_items__item__restaurant_id=pk
    ).values_list('id', flat=True)
    
    restaurant_reviews = review.objects.filter(order__id__in=restaurant_order_ids)
    average_rating = restaurant_reviews.aggregate(avg_rating=Avg('rating'))['avg_rating']
    average_rating = average_rating if average_rating!=None else 0
    ...
    return render(request, 'home/restaurant_view.html',{'restaurant':restaurant, 'menus':lmenus,'cart':cart,'pp':pp, 'reviews':restaurant_reviews, 'avg_rating':average_rating})
...
def sign_up(request, data=None, err=None):
    cart = []
    pp = 0
    if 'cart' in request.session:
        for c in request.session['cart']:
    ...
    if "home_login" in request.session:
        return redirect(reverse('home:home'))
    return render(request, 'home/sign_up.html',{'cart':cart,'pp':pp,'data':data, 'err':err})

def logout(request):
    if "home_login" in request.session:
        del request.session['home_login']
    if 'calories' in request.session:
...
def register(request):
    if request.method == 'POST':
        ...
        # user = model_to_dict(user if not user == None else {})
        ...
        if not user:
            ...
            return redirect(reverse('home:sign_up', kwargs={'data': request.POST, 'err': True}))
        if user:
            request.session['home_login'] = user.email
            return redirect(reverse('home:profile'))
...
def past_orders(request):
    ...
    for o in porders:
        ...
        reviews = review.objects.filter(order=orders.objects.get(id=o.id)).first()
        borders.append({'order':o,'items':oitems,'reviews':reviews})
    
    return render(request, 'home/user_orders.html',{'orders':porders, 'borders':borders, 'user':user,'cart':cart,'pp':pp})

def review_experience(request):
    if not 'home_login' in request.session:
        return redirect(reverse('home:sign_in'))
    data = request.POST
    user = users.objects.filter(email=request.session['home_login']).first()
    reviews = review.objects.create(order_id = data.get('order'),text=data.get('feedback'),rating =data.get('rating'))
    return redirect(reverse('home:past_orders'))